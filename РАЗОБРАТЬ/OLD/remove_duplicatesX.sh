...
# 3. Поиск дубликатов
echo "Поиск дубликатов через fdupes..."
fdupes -r -n -S "$(pwd)" > duplicates.txt

# 4. Анализ качества и удаление
...
print("Обработка групп дубликатов...")
current_group = []
...
deactivate
rm -f duplicates.txt
echo -e "\nГотово! Все дубликаты обработаны."
echo "Виртуальное окружение сохранено в $VENV_DIR (можно удалить командой 'rm -rf $VENV_DIR')"
